...
const props = defineProps({
  role: {
    ...
    validator: (value) => ['admin', 'gv', 'dbcl', 'tbm', 'tk'].includes(value)
  },
  title: {
    type: String,
    default: ''
  ...
  <div class="admin-dashboard">
    ...
    <div class="sidebar" :class="{ 'sidebar-hidden': !isSidebarVisible }">
      ...
      <ul v-else-if="role === 'tk'" class="sidebar-menu">
        <li><i class="fas fa-users"></i> <Link :href="route('tk.dsdangky.index')">Danh sách đăng ký biên soạn</Link></li>
        <li><i class="fas fa-users"></i> <Link :href="route('tk.dsbienban.index')">Danh sách biên bản họp cấp khoa</Link></li>
      </ul>
    ...
    <div class="main-content">
      ...
      <div class="header">
        ...
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">{{ roleTitle }}</a></li>
            <slot name="sub-link" />
          </ol>
        </nav>
        <slot name="message" />
